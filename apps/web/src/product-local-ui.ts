...
import { readM3BuildNetwork } from './m3-network.ts';
...
const networkConfig = readM3BuildNetwork(import.meta.env);
const AF = window.AF;
...
if (!onchainRuntime && import.meta.env.DEV && new URLSearchParams(location.search).get('m3Fixture') === '1') {
  ...
  const fixture = fixtureModule.createM3InjectedRuntimeFixture(networkConfig);
...
installM3WalletControls(AF, onchainRuntime, run, (message) => {
  localError = message;
  const out = document.querySelector('[data-product-dialog-error]');
