  ...
  async (t) => {
    ...
    for (const index of [0, 12, 23]) {
      ...
      for (const key of ['open', 'high', 'low', 'close'])
        assert.equal(await field(key), money(row[key]) + ' USDT');
      assert.equal(await field('change'), signed(row.close - row.open) + ' USDT');
      ...
      assert.equal(await field('turnover'), money(row.quoteVolume) + ' USDT');
      assert.equal(
