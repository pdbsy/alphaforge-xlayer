...
test('upstream records and cross-repository ACKs cannot confirm X Layer work', () => {
  const notice = source();
  const ack = source({ pr: 2, owner: 'Macbeth02', type: 'ACK', to: 'Macbeth01', replyTo: notice.source_url });
  const historical = JSON.parse(
    JSON.stringify(ack).replaceAll('pdbsy/alphaforge-xlayer', 'pdbsy/quantpass-arbitrum-hackathon'),
  );
  const mixed = { ...ack, pr_head_repo: 'pdbsy/quantpass-arbitrum-hackathon' };
  const crossLink = {
    ...ack,
    text: ack.text.replace(
      notice.source_url,
      notice.source_url.replace('alphaforge-xlayer', 'quantpass-arbitrum-hackathon'),
    ),
  };
  const snapshot = buildForumSnapshot([notice, historical, mixed, crossLink]);
  assert.equal(snapshot.source.state, 'PARTIAL');
  assert.equal(snapshot.source.rejected_records, 3);
