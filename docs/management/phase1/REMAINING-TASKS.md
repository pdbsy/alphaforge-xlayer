...
| PH1-04 | 用户03；CONFIG-001/ASSET-001/ADAPTER-001/RPC-001 | manifest、地址/字节码/ABI、三方链身份与调用目标校验 | VERIFY_EXISTING_THEN_FIX | 03 / 02/04/05 | base；接口变化依赖PH1-02 | 配置/适配回归与接入说明；错链/错地址/错ABI/错owner失败关闭；真实参数缺失保持 NOT_DEPLOYED | packages/chain-adapter/；新03报告 | IMPLEMENTATION_EVIDENCE / 否 |
...
| PH1-09 | 用户04；Pass/Vault端到端 | 发行/分配或获取 Pass、自由转账、创建/选择 Vault 操作入口 | MUST_COMPLETE | 04 / 02/03/05 | PH1-02/04；PH1-16已明确仅首发分配与转账 | 操作闭环、精确接口和浏览器用例；1 PASS容量不等于价格；转账18位；创建显式Owner；已支持路径可复現 | 新02/03/04交接 | IMPLEMENTATION / 否 |
