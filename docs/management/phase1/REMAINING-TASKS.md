...
| PH1-09 | 用户04；Pass/Vault端到端 | 发行/分配或获取 Pass、自由转账、创建/选择 Vault 操作入口 | DELIVERED_PENDING_INDEPENDENT_ACCEPTANCE | 04 / 02/03/05 | PH1-02/04；PH1-16已明确仅首发分配与转账 | 操作闭环、精确接口和浏览器用例；1 PASS容量不等于价格；转账18位；创建或选择 Vault；受审 allowlist 选择满足产品路径，创建保留显式 Owner 部署流程；切换废弃旧意图/模拟 | PR27 ffe7d08 已集成；reviewed allowlist选择、切换审阅失效、shared-Pass双Vault浏览器自证通过；05独立验收进行中 | CANDIDATE_DEPENDENCY / 否 |
...
| PH1-12 | 用户05；DoD覆盖率 | 功能验收矩阵、准确覆盖率与跨层独立复测 | IN_PROGRESS_NOT_COMPLETE | 05 / 02/03/04 | 起点base；最终统一候选 | 独立功能复测/覆盖率/缺陷及修复SHA；测量总体90%目标和关键授权会计100%分支要求；不足仍FAIL/BLOCKED，不拿数量替代 | 05最新c27c869已集成；03后续a4d73bb关键清单独立通过；04新选择器独立待验；总体/浏览器NOT_MEASURED，P1-001 OPEN | BROWSER_MEASUREMENT_AND_CANDIDATE_DEPENDENCY / 否 |
...
| PH1-14 | 用户06；CI-GATES/SECRET-001/SUPPLY-001 | 7必需CI、附加任务、合约/scanner覆盖和证据真实性 | BLOCKED_EXACT_FALSE_POSITIVE_DECISION | 06 / 01/05 | base；每个准确候选 | 六项分列结论和run/job/step证据；9jobs每关键步骤实际执行；错误失败关闭；旧CI失败不改写 | 中间df23817两组18jobs/204steps通过；新来源引入历史Git tree单条误报，GITLEAKS-FP-001精确处置方案已准备并询问用户；06独立对象证明一致 | GITLEAKS_FALSE_POSITIVE_POLICY_DECISION / 是 |
