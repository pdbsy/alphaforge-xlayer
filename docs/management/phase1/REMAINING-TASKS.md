...
经理已正常集成最新03 a4d73bb、04 ffe7d08、05 c27c869来源；本地集成705/705及类型/静态/格式/隐私/构建通过。此前中间df23817的两组CI共18jobs/204steps全部成功，不能代替新候选。05已独立通过PR24合约核心和PR26冻结关键分支清单；04新选择器仍在独立验收。新历史Git tree误报的精确处置等待用户决定，总体/浏览器覆盖率仍NOT_MEASURED。原临时日志不可用，不能冒充当前证据。详见 [恢复与当前回执](Macbeth01-RESUMPTION.md)；完整字段见 [JSON矩阵](remaining-tasks.json)。
...
| PH1-02 | 当前用户02；CON-001/SPEC-001/SPEC-002/ABI-001 | Pass 固定供应、策略绑定、完整权限/事件/ABI；只补缺 | PASS_AT_PR24_SOURCE_CHECKPOINT | 02 / 03/04/05 | base ABI | 覆盖表、必要修复和 ABI handoff；25 Vault selectors/7 topics 等完整 compiler equality；发行/转账精度及异常 token 回归 | PR24 a130529；05独立134/134 Solidity、24/24 Python、Slither和核心Vault/Locker/Pass 100%已通过；统一候选待复跑 | CANDIDATE_DEPENDENCY / 否 |
| PH1-03 | DRYRUN-001/SEC-002；用户02 | 可重建构建、constructor/immutables/bytecode 清单和本地 VM 演练 | PASS_AT_PR24_SOURCE_CHECKPOINT | 02 / 03/05/06 | PH1-02 | 构建清单、离线参数、local VM 演练；干净重建一致；重复运行不覆盖；明确无外部链广播 | PR24 a130529；05独立ABI/manifest/local deployment rehearsal通过；无真实Testnet部署 | CANDIDATE_DEPENDENCY / 否 |
| PH1-04 | 用户03；CONFIG-001/ASSET-001/ADAPTER-001/RPC-001 | manifest、地址/字节码/ABI、三方链身份与调用目标校验 | PASS_AT_PR26_SOURCE_CHECKPOINT | 03 / 02/04/05 | base；接口变化依赖PH1-02 | 配置/适配回归与接入说明；错链/错地址/错ABI/错owner失败关闭；真实参数缺失保持 NOT_DEPLOYED | PR26 a4d73bb；05独立643/643，冻结Chain/API关键授权会计分支清单已通过；整体13文件95.39%分支保持原值 | CANDIDATE_DEPENDENCY / 否 |
...
| PH1-06 | 用户03；INDEX-001 | 有界 catch-up、重组、跨进程所有权和多实体隔离 | DELIVERED_PENDING_FINAL_ACCEPTANCE | 03 / 05 | base索引实现 | 重启/竞争/多Vault多钱包多策略回归；同块多操作不丢失；无共同祖先保留证据且持久degraded | PR26 a4d73bb 已集成；共享Pass隔离及关键竞态分支已独立验证，统一候选待复跑 | CANDIDATE_DEPENDENCY / 否 |
...
| PH1-09 | 用户04；Pass/Vault端到端 | 发行/分配或获取 Pass、自由转账、创建/选择 Vault 操作入口 | PASS_AT_PR27_SOURCE_CHECKPOINT | 04 / 02/03/05 | PH1-02/04；PH1-16已明确仅首发分配与转账 | 操作闭环、精确接口和浏览器用例；1 PASS容量不等于价格；转账18位；创建或选择 Vault；受审 allowlist 选择满足产品路径，创建保留显式 Owner 部署流程；切换废弃旧意图/模拟 | PR27 ffe7d08；05独立86/86与137/137及真实多Vault浏览器通过；统一639ffd8的705/705及浏览器亦独立通过，新C/R/S仍待完成 | CANDIDATE_DEPENDENCY / 否 |
...
| PH1-12 | 用户05；DoD覆盖率 | 功能验收矩阵、准确覆盖率与跨层独立复测 | IN_PROGRESS_NOT_COMPLETE | 05 / 02/03/04 | 起点base；最终统一候选 | 独立功能复测/覆盖率/缺陷及修复SHA；测量总体90%目标和关键授权会计100%分支要求；不足仍FAIL/BLOCKED，不拿数量替代 | 05独立639ffd8全套705/705和多Vault浏览器通过；冻结Chain/API关键子集通过；总体仍NOT_MEASURED，隔离mapper资格验证进行中，P1-001 OPEN | BROWSER_MEASUREMENT_AND_CANDIDATE_DEPENDENCY / 否 |
...
| PH1-14 | 用户06；CI-GATES/SECRET-001/SUPPLY-001 | 7必需CI、附加任务、合约/scanner覆盖和证据真实性 | APPROVED_DISPOSITION_PENDING_VERIFICATION | 06 / 01/05 | base；每个准确候选 | 六项分列结论和run/job/step证据；9jobs每关键步骤实际执行；错误失败关闭；旧CI失败不改写 | 用户已批准GITLEAKS-FP-001精确历史误报例外；实现26/26针对回归通过；待准确新候选真实scanner、CI与06独立验证，旧失败记录保留 | NEW_CANDIDATE_SCANNER_AND_CI_VERIFICATION / 否 |
